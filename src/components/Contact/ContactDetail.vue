...
async function fetchAddresses() {
    // Fetch addresses for the contact
    const response = await AddressList(token.value, id);
    const responseBody = await response.json();
    console.log('Fetch Addresses response:', responseBody);
...
async function fetchContact() {
    ...
    const responseBody = await response.json();
    console.log('Fetch contact response:', responseBody);
    if (response.status === 200) {
...
async function handleDeleteAddress(addressId) {
    ...
    // This function can be used to delete the address
    const confirmed = await alertConfirm('Are you sure you want to delete this address?');
    if (!confirmed) return;
    const response = await AddressDelete(token.value, id, addressId);
    const responseBody = await response.json();
    console.log('Delete address response:', responseBody);
    if (response.status === 200) {
        await alertSuccess('Address deleted successfully');
        // Optionally refresh the addresses list
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        class="bg-gray-700 bg-opacity-50 p-5 rounded-lg shadow-md border border-gray-600 card-hover">
                            ...
                            <div class="flex justify-end space-x-3">
                                <RouterLink :to="`/dashboard/contacts/${id}/addresses/${address.id}/edit`" class="px-4 py-2 bg-gradient text-white rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-md flex items-center">
