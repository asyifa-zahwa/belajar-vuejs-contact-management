...
async function fetchAddress() {
    ...
    console.log('Fetch address response:', responseBody);
...
async function handleSubmit() {
    ...
    // This function can be used to save the new address details
    const response = await AddressUpdate(token.value, id, addressId, address);
    const responseBody = await response.json();
    console.log('Create address response:', responseBody);
    if (response.status === 200) {
        await alertSuccess('Address created successfully');
        // Optionally redirect to the contact detail page
        // await router.push({
...
<div class="flex items-center mb-6">
            <RouterLink :to="`/dashboard/contacts/${id}`" class="text-blue-400 hover:text-blue-300 mr-4 flex items-center transition-colors duration-200">
